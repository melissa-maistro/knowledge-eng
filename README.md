...
│   │   ├── flavordb/           ← flavordb_entities.csv, flavordb_molecules.csv, flavordb_links.csv
│   │   ├── usda/               ← food.csv, nutrient.csv, food_nutrient.csv
│   │   ├── off/                ← en.openfoodfacts.org.products.csv
│   │   └── foodcom/            ← RAW_recipes.csv
...
2. **USDA** is matched to canonical names by fuzzy-matching ingredient names against USDA Foundation Foods descriptions (root name before the first comma). 99/935 matched with high confidence.
...
- Canonical ingredient list built with 99 USDA matches and 935 Food.com appearance counts
...
2. **USDA coverage is 99/935 (11%).** Foundation Foods only has 469 entries. SR Legacy would improve this but contains branded foods that corrupt fuzzy matching. This limits the nutritional similarity signal.
3. **No culinary role signal.** Zucchini scores as the top wheat substitute because it shares many volatile molecules, but it is not a flour substitute. A culinary role layer (fat, flour, binder, acid, sweetener etc.) is needed to fix this.
4. **Food.com co-occurrence is zero.** Food.com ingredient strings ("2 cups all-purpose flour") don't match canonical names ("Flour") exactly enough to produce co-occurrence pairs. Needs a normalisation pass on Food.com strings.
5. **Open Food Facts allergen coverage is for packaged products only.** Whole ingredients like butter, olive oil, garlic have no OFF entries — allergen data for these needs to be added separately.

---

## What needs to be done next

### Priority 1 — Culinary role layer
Build `pipelines/culinary_roles.py` that assigns each canonical ingredient a culinary role (fat, flour, sweetener, acid, binder, dairy_liquid, aromatic, protein, thickener, leavening). Add role-based similarity as a fourth signal in `similarity.py`. This fixes the Butter ↔ Olive oil problem and the Wheat ↔ Zucchini problem.

Three options discussed:
- **Option A**: Derive from FlavorDB `category` field (fastest, least accurate)
- **Option B**: Derive from Food.com recipe context using NLP (most principled, most work)
- **Option C**: Small hand-curated CSV with `ingredient_name, culinary_role` (~200 rows) — most auditable for a clinical application
...
| File | Where | Place in |
|------|-------|----------|
| USDA Foundation Foods ZIP | https://fdc.nal.usda.gov/download-datasets | `data/raw/usda/` |
| Open Food Facts CSV | https://world.openfoodfacts.org/data | `data/raw/off/` |
| Food.com RAW_recipes.csv | https://www.kaggle.com/datasets/shuyangli94/food-com-recipes-and-user-interactions | `data/raw/foodcom/` |

FlavorDB CSVs should already be in `data/raw/flavordb/` — ask the person who ran the scraper.
